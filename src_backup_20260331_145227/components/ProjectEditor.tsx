      ...
      <div className="flex items-start gap-1 p-1.5">
        ...
        <div className="flex-1 min-w-0" onClick={onClick}>
          ...
          <div className="flex items-center gap-1 mt-1 px-0.5">
            <span className="text-[9px] font-bold text-gray-300 tabular-nums flex-shrink-0">{idx + 1}</span>
...
export function ProjectEditor({ projectId, onBack }: Props) {
  ...
  async function handleTemplateSelect(_: Id<"templates"> | null, builtinSlides?: Array<{ type: string; title: string; content: string }>) {
    if (builtinSlides) {
      await bulkInsert({ projectId, slides: builtinSlides.map((s, i) => ({ ...s, order: slides.length + i })) });
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0 flex items-center gap-2">
            ...
            {project?.targetCompany && (
              <span className="text-xs text-gray-400 truncate hidden md:block max-w-[120px]">· {project.targetCompany}</span>
            )}
            {project?.dealType && (
              <span
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-1.5 space-y-1">
            ...
            {slides.length === 0 && (
              <div className="text-center py-10 px-2">
                ...
                <button onClick={addSlide} className="mt-2 text-[10px] font-semibold hover:underline" style={{ color: theme.primaryColor }}>
                  + Ajouter
                </button>
              </div>
            )}
        ...
        <div className="w-[300px] bg-white border-l border-gray-100 flex flex-col flex-shrink-0">
          ...
          <div className="flex border-b border-gray-100 flex-shrink-0">
            {PANEL_BUTTONS.map(btn => (
              ...
              >
                ...
                <span className="text-[9px] font-semibold leading-tight">{btn.label.split(" ")[0]}</span>
              </button>
            ))}
          </div>
          ...
          <div className="flex-1 overflow-hidden">
            ...
            {activePanel === "search" && (
              <SearchPanel projectId={projectId} onSelectSlide={setActiveSlideIdx} slides={slides} />
            )}
            {activePanel === "uploads" && <UploadPanel projectId={projectId} />}
            {activePanel === "settings" && <ProjectSettings projectId={projectId} project={project} />}
            {activePanel === null && (
              <div className="flex items-center justify-center h-full">
                <div className="text-center px-6">
                  ...
                  <p className="text-[10px] text-gray-300 mt-1">IA, éléments, recherche, fichiers ou réglages</p>
                </div>
              </div>
            )}
          </div>
