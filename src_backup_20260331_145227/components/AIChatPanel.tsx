...
const QUICK_PROMPTS = [
  { label: "Générer le deck complet", icon: "✨", special: "generate", color: "#1a3a5c" },
  { label: "Améliorer le résumé exécutif", icon: "📋", prompt: "Améliore et enrichis le résumé exécutif avec des éléments plus percutants et différenciants. Fournis le contenu révisé en JSON slides." },
  { label: "Renforcer la thèse d'investissement", icon: "💡", prompt: "Renforce la thèse d'investissement avec des arguments plus convaincants et des données de marché pertinentes. Fournis le contenu révisé en JSON slides." },
  { label: "Projections financières", icon: "📊", prompt: "Propose des diapositives de projections financières détaillées avec CA, EBITDA, croissance et multiples de valorisation. Fournis en JSON slides." },
  { label: "Acquéreurs potentiels", icon: "🎯", prompt: "Identifie et présente des acquéreurs stratégiques et financiers potentiels pour cette opération. Fournis en JSON slides." },
  { label: "Analyse de marché TAM/SAM/SOM", icon: "🌍", prompt: "Crée une analyse de marché approfondie avec TAM/SAM/SOM, tendances et dynamiques concurrentielles. Fournis en JSON slides." },
  { label: "Équipe dirigeante", icon: "👥", prompt: "Crée une diapositive équipe dirigeante professionnelle avec les profils clés. Fournis en JSON slides." },
  { label: "Calendrier du processus", icon: "📅", prompt: "Propose un calendrier de processus M&A réaliste avec les étapes clés. Fournis en JSON slides." },
];

function MessageContent({ content, onApplySlides }: { content: string; onApplySlides: (content: string) => void }) {
...
export function AIChatPanel({ projectId, project, slides }: Props) {
  ...
  const projectContext = project
    ...
    : "";
  const slidesContext = slides.length > 0
    ? slides.map((s, i) => `${i + 1}. [${s.type}] "${s.title}": ${s.content.substring(0, 150)}`).join("\n")
    : "Aucune diapositive";
  ...
  async function handleSend(text?: string) {
    ...
    try {
      await sendMessage({ projectId, content: msg, projectContext, slidesContext });
    } catch {
  ...
  async function handleGenerateDeck() {
    ...
    setShowQuickPrompts(false);
    try {
      await generateDeck({ projectId, projectContext });
  ...
  async function applySlides(content: string) {
    ...
    if (!match) { toast.error("Format de diapositives invalide"); return; }
    try {
      const parsed = JSON.parse(match[1]);
      if (Array.isArray(parsed)) {
        await bulkInsert({
          projectId,
          slides: parsed.map((s: { type: string; title: string; content: string }, i: number) => ({
            ...s,
      ...
      toast.error("Impossible d'appliquer les diapositives — format JSON invalide");
    }
  }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2.5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-[#1a3a5c]/5 to-transparent flex-shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-xs font-bold text-[#1a3a5c] leading-tight">Assistant IA M&A</p>
            <p className="text-[10px] text-gray-400 leading-tight truncate max-w-[140px]">{providerLabel}</p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          ...
          {messages.length > 0 && (
            <button
              onClick={() => { clearHistory({ projectId }); setShowQuickPrompts(true); }}
      ...
      {showQuickPrompts && (
        <div className="border-b border-gray-100 bg-white flex-shrink-0">
          <div className="px-3 pt-2.5 pb-1">
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-2">⚡ Actions rapides</p>
          </div>
          <div className="px-2 pb-2 space-y-1 max-h-[220px] overflow-y-auto">
            {QUICK_PROMPTS.map(p => (
              <button
                key={p.label}
                onClick={() => p.special === "generate" ? handleGenerateDeck() : handleSend(p.prompt)}
                disabled={sending}
              ...
              >
                ...
                <svg className="w-3 h-3 ml-auto opacity-0 group-hover:opacity-100 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && !showQuickPrompts && (
          <div className="text-center py-10 text-gray-400">
            ...
            <p className="text-xs font-medium">Posez une question à l'assistant IA</p>
            <p className="text-[10px] mt-1 text-gray-300">Spécialisé en M&A, fusions-acquisitions et pitch decks</p>
          </div>
        )}
        {messages.map(msg => (
          <div key={msg._id} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : "justify-start"} animate-fade-in`}>
            {msg.role === "assistant" && (
            ...
            <div className={`max-w-[88%] rounded-2xl px-3 py-2.5 ${
              ...
              {msg.role === "assistant" ? (
                <MessageContent content={msg.content} onApplySlides={applySlides} />
              ) : (
