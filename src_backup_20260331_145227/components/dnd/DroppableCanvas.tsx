...
export const DroppableCanvas: React.FC<DroppableCanvasProps> = ({
  ...
  const handleDragOverNative = useCallback((event: React.DragEvent) => {
    event.preventDefault();
    if (!readOnly) {
      setIsOver(true);
    }
  ...
  return (
    ...
    >
          ...
          <div className="drop-overlay">
            <div className="drop-indicator">
              ...
              <span className="drop-text">
                {dragOverType 
                  ? `Déposer le bloc ${dragOverType.toLowerCase()} ici`
                  : 'Déposer ici'
                }
              </span>
            </div>
      ...
      <div className="canvas-info">
        ...
        <span className="info-item">
          {snapToGrid ? `Grille ${gridSize}px` : 'Position libre'}
        </span>
      </div>
...
function createDefaultBlock(type: BlockType, position: { x: number; y: number }): BlockData {
  const defaultSizes: Record<BlockType, { width: number; height: number }> = {
    ...
    'Graphique': { width: 400, height: 250 },
    'Tableau': { width: 500, height: 200 },
    'Deux colonnes': { width: 600, height: 200 },
...
function renderBlockContent(block: BlockData): React.ReactNode {
  switch (block.type) {
    ...
    case 'Tableau':
      return (
        <div style={{ width: '100%', border: '1px solid #e0e0e0', borderRadius: '4px', overflow: 'hidden' }}>
