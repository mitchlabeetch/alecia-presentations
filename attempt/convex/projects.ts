...
export const list = query({
  ...
  handler: async (ctx) => {
    ...
    if (!userId) return [];
    return await ctx.db.query("projects").withIndex("by_owner", q => q.eq("ownerId", userId)).order("desc").collect();
  },
});
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const projectId = await ctx.db.insert("projects", {
      ...
      theme: { primaryColor: "#1a3a5c", accentColor: "#c9a84c", fontFamily: "Inter" },
