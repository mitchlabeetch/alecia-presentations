...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f7f8fa]" style={{ fontFamily: "'Inter', sans-serif" }}>
      <Authenticated>
        {activeProjectId ? (
          <ProjectEditor
...
function LandingPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "linear-gradient(135deg, #0f2035 0%, #1a3a5c 50%, #1e4a6e 100%)" }}>
      {/* Header */}
      <header className="h-16 flex items-center justify-between px-8 border-b border-white/10">
        ...
        <div className="hidden sm:flex items-center gap-5 text-sm text-white/60">
          <span className="flex items-center gap-1.5"><span className="text-[#c9a84c]">✦</span> IA intégrée</span>
          <span className="flex items-center gap-1.5"><span className="text-[#c9a84c]">✦</span> Temps réel</span>
          <span className="flex items-center gap-1.5"><span className="text-[#c9a84c]">✦</span> Collaboratif</span>
        </div>
      </header>
        ...
        <div className="w-full max-w-[420px]">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-white/60 text-sm leading-relaxed">
              Pitch decks M&A professionnels<br />propulsés par l'intelligence artificielle
            </p>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-6">
            {["Cession", "LBO", "Acquisition", "Levée de fonds", "Fusion"].map(t => (
              <span key={t} className="px-3 py-1 rounded-full bg-white/10 border border-white/20 text-xs text-white/70 backdrop-blur">{t}</span>
            ))}
          </div>
